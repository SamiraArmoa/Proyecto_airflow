...
/* BUSCAR VUELOS POR ORIGEN Y DESTINO PARA SOCKETS */
char *db_vuelos_buscar_origen_destino_texto(void *db, const char *origen, const char *destino) {
    ...
    sqlite3_prepare_v2(DB(db),
        ...
        "LEFT JOIN Aeropuerto a1 ON v.ID_ORIGEN = a1.ID_A "
        "LEFT JOIN Aeropuerto a2 ON v.ID_DESTINO = a2.ID_A "
        "WHERE a1.CODIGO = ? AND a2.CODIGO = ?",
        -1, &stmt, NULL);

    sqlite3_bind_text(stmt, 1, origen, -1, SQLITE_TRANSIENT);
...
int db_usuario_buscar_email(void *db,
                            const char *email,
                            int *id_out,
                            char *nombre_out,
                            long long *tlf_out,
    ...
    sqlite3_prepare_v2(DB(db),
        "SELECT ID_US, NOMBRE, TLF, CONTRASENA, ROL "
    ...
    if (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        if (tlf_out)
            *tlf_out = sqlite3_column_int64(stmt, 2);

        if (contrasena_out)
            strcpy(contrasena_out, (const char *)sqlite3_column_text(stmt, 3));

        if (rol_out)
            *rol_out = sqlite3_column_int(stmt, 4);
...
char *db_comprar_billete_texto(void *db,
    ...
    if (sqlite3_step(stmt) != SQLITE_DONE) {
        strcpy(resultado, "ERROR|No se pudo comprar el billete");
        sqlite3_finalize(stmt);
        return resultado;
    }
...
char *db_billetes_usuario_texto(void *db, int id_usuario) {
    sqlite3_stmt *stmt;
    char *resultado = (char *)malloc(20000);
    ...
    sqlite3_prepare_v2(DB(db),
        "SELECT b.ID_BILLETE, v.COD_VUELO, "
        "a1.CODIGO, a2.CODIGO, "
        "v.FECHA_HORA, b.ASIENTO, b.FECHA_COMPRA "
        "FROM Billete b "
    ...
    append_text(resultado, 20000,
        "\nMIS BILLETES:\n");
    append_text(resultado, 20000,
        "ID | VUELO | ORIGEN | DESTINO | FECHA VUELO | ASIENTO | FECHA COMPRA\n");
    ...
    if (encontrados == 0) {
        strcpy(resultado, "ERROR|No tienes billetes comprados");
    }
...
int db_limpiar_billetes(void *db) {
        return exec_simple(DB(db), "DELETE FROM Billete;");
}